...
::: notes
\tiny
- i said on the previous slide that there was a section of the genome that didn't have sufficient coverage to generate one long sequence
...
- in the next video i'll also talk about why we get gaps in the assembly in a bit more detail
...
- we can also use the k-mer profiles to spot reads that have errors in them, because erroneous k-mers will be rare in the whole dataset
- so that allows us to spot kmers with errors, and just dump them from the kmer table
:::
...
- hopefully what you can see is that these two situations can be resolved by generating longer sequencing reads that span these structures, and that's why there's been a big push to increase the length, throughput and accuracy of long read sequencing
:::
...
::: notes
\tiny
- now that we've looked at what a genome sequence is and why we might want one, and got a handle on some of the concepts of genome assembly, it's time to take a brief look at how genome assembly software works
- first we need to be a bit more precise about what we mean by a genome sequence
...
- that's the slighly more precise definition of a genome that we need to keep in mind when we're looking at assembly algorithms
...
- another factor to consider here is ploidy
...
- spades is a very popular assembler and it's one of the ones you'd probably try first if you were doing a short read assembly
...
- unicycler is specifically for hybrid assembly of circular sequences like bacterial chromosomes
